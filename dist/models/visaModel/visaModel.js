...
class VisaModel extends schema_1.default {
    ...
    getVisaList(query) {
        return __awaiter(this, void 0, void 0, function* () {
            const queryResult = yield this.db('visa as v')
                .withSchema(this.SERVICE_SCHEMA)
                .select('v.id', 'c.nice_name as country_name', 'v.title', 'v.visa_type', 'v.visa_mode', 'v.max_validity', 'v.image')
    ...
    getSingleVisa(query) {
        return __awaiter(this, void 0, void 0, function* () {
            return this.db('visa')
                .withSchema(this.SERVICE_SCHEMA)
                .select('id', 'title', 'slug', 'description', 'image', 'status', 'country_id', 'visa_type', 'visa_mode', 'visa_fee', 'processing_fee', 'max_validity', 'stay_validity', 'documents_details', 'required_fields', 'meta_title', 'meta_description', 'visa_for')
                .where((qb) => {
                qb.andWhere('is_deleted', query.is_deleted);
                qb.andWhere('source_id', query.source_id);
                qb.andWhere('source_type', query.source_type);
