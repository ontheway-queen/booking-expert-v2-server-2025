...
export default class MoneyReceiptModel extends Schema {
    ...
    public async updateMoneyReceipt(payload: IUpdateMoneyReceiptPayload, id: number) {
        return await this.db("money_receipt")
            .withSchema(this.DBO_SCHEMA)
            .update(payload)
            .where({ id });
    ...
    public async getSingleMoneyReceipt(params: IGetSingleMoneyReceiptParams): Promise<IGetMoneyReceiptData> {
        return await this.db("money_receipt")
            .withSchema(this.DBO_SCHEMA)
            .select("*")
            .where({ id: params.id })
