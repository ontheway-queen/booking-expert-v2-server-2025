...
import {
  ICreateCurrencyPayload,
  IUpdateCurrencyPayload,
} from '../../utils/modelTypes/othersModelTypes/currencyModel.interface';
...
class CurrencyModel extends Schema {
  ...
  public async createApiWise(payload: ICreateCurrencyPayload) {
    return await this.db('api_wise_currency')
      .withSchema(this.DBO_SCHEMA)
      .insert(payload, 'id');
  }

  public async updateApiWise(payload: IUpdateCurrencyPayload, id: number) {
    return await this.db('api_wise_currency')
      .withSchema(this.DBO_SCHEMA)
      .update(payload)
      .where({ id });
