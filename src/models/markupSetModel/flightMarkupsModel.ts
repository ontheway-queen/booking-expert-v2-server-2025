...
import {
  ICreateFlightMarkupsPayload,
  IGetApiActiveAirlinesData,
...
export default class FlightMarkupsModel extends Schema {
  ...
  public async createFlightMarkups(
    payload: ICreateFlightMarkupsPayload | ICreateFlightMarkupsPayload[]
  ): Promise<number[]> {
  ...
  public async getAllFlightMarkups(
    ...
    const data = await this.db('flight_markups as fm')
      ...
      .leftJoin(
        'set_flight_api_view AS fa',
        'fm.markup_set_flight_api_id',
        'fa.id'
      )
      .joinRaw('left join ?? on ?? = ??', [
      ...
      .where((qb) => {
        ...
        if (query.markup_set_flight_api_id) {
          qb.andWhere(
            'fm.markup_set_flight_api_id',
            query.markup_set_flight_api_id
          );
        }
        if (query.airline) {
  ...
  public async getAPIActiveAirlines(
    markup_set_flight_api_id: number
  ): Promise<IGetApiActiveAirlinesData[]> {
    return await this.db('flight_markups')
      .withSchema(this.DBO_SCHEMA)
  ...
  public async updateFlightMarkups(
    payload: IUpdateFlightMarkupsPayload,
    id: number
  ) {
    return await this.db('flight_markups')
      .withSchema(this.DBO_SCHEMA)
