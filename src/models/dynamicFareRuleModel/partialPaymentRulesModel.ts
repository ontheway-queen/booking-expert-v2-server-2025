...
import {
  ICreatePartialPaymentRulePayload,
  IGetPartialPaymentRuleQueryFilter,
  IUpdatePartialPaymentRulePayload,
} from '../../utils/modelTypes/dynamicFareRulesModelTypes/partialPaymentRulesModel.interface';
...
class PartialPaymentRuleModel extends Schema {
  ...
  public async getAll(
    query: IGetPartialPaymentRuleQueryFilter,
  ...
  public async getPartialPaymentCondition(payload: {
    ...
    const data = await this.db('partial_payment_rules')
      ...
      .where((qb) => {
        qb.where('supplier.api', payload.flight_api_name).andWhere(
          (builder) => {
            builder
        ...
        if (payload.travel_date) {
          const travelDateFromNow = Math.ceil(
            (new Date(payload.travel_date).getTime() - new Date().getTime()) /
              (1000 * 60 * 60 * 24)
          );
          qb.andWhere(
            'partial_payment_rules.travel_date_from_now',
