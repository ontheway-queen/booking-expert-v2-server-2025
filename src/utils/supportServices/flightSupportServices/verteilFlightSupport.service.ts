...
import VerteilRequests from '../../lib/flight/verteilRequest';
...
import {
  ...
  IVerteilOrderReshopRQ,
  IVerteilOrderReshopRS,
  IVerteilOrderRetrieveRQ,
} from '../../supportTypes/flightTypes/verteilFlightTypes';
import {
  FLIGHT_TICKET_ISSUE,
  ROUTE_TYPE,
  VERTEIL_API,
...
export default class VerteilFlightService extends AbstractServices {
  ...
  private flightSearchRequestFormatter(
    ...
    return {
      Party: deal_code
        ? { Sender: { CorporateSender: { CorporateCode: deal_code } } }
        : undefined,
      Preference: {
        CabinPreferences: {
          CabinType: [
            {
              ...
              Code:
                CTC === '4' ? 'F' : CTC === '3' ? 'C' : CTC === '2' ? 'W' : 'Y', // Economy-Y PremiumEconomy-W BusinessClass-C FirstClass-F
            },
          ],
        },
        FarePreferences: {
  ...
  public async FlightSearchService({
    ...
    const AirShoppingRS = await this.request.postRequest(
      ...
      AirShoppingRQ,
      airlineCodesCsv
        ? { headers: { ThirdpartyId: airlineCodesCsv } }
        : undefined
    );
    // const AirShoppingRS = await this.request.postRequest<
    //     IVerteilFlightSearchRQ,
    ...
    {
      const flightPriceRQs =
        await new VerteilFlightUtils().PrepareMetaDataForFlightPrice(
          reqBody,
          result,
          AirShoppingRS,
          AirShoppingRQ
  ...
  private async FlightSearchResFormatter({
    ...
    const leg_description = reqBody.OriginDestinationInformation.map(
      (OrDeInfo) => {
        return {
      ...
      try {
        ...
        const FormattedFlights = await Promise.all(
          Associations.map(async (Association) => {
              ...
              SegmentRefs.map(async (segRef) => {
                ...
                let elapsed_time: number;
                {
                  const SegmentDuration =
                    FlightSegment.FlightDetail?.FlightDuration.Value;
                  if (SegmentDuration) {
                    elapsed_time =
                      Duration.fromISO(SegmentDuration).as('minutes');
                  } else {
                    const dDt = DateTime.fromISO(
                      departure.date + 'T' + departure.time
                    );
                    const aDt = DateTime.fromISO(
                      arrival.date + 'T' + arrival.time
                    );
            ...
            const FLightInfo = data.DataLists.FlightList?.Flight.find(
              (item) => {
                const flightKeyMatch =
                  item.FlightKey ===
                  Association.ApplicableFlight.FlightReferences.value[0];
            ...
            if (FLightInfo?.Journey) {
              ...
              flightElapsedTime =
                Duration.fromISO(FlightDuration).as('minutes');
            }
        ...
        const FormattedAvailability = Associations.map(
          ...
          (Association, AssociationIndex) => {
            ...
            const OriginDestination =
              data.DataLists.OriginDestinationList.OriginDestination.find(
            ...
            const SegmentRefs =
              Association.ApplicableFlight.FlightSegmentReference; // Segments for this LEG
            ...
            const FormattedSegments = SegmentRefs.map(
              ...
              (Segment, SegmentIndex) => {
                ...
                const FormattedPassengers = Offer.PricedOffer.OfferPrice.map(
                  ...
                  (OfferPrice) => {
                    const travelerRef =
                      OfferPrice.RequestedDate.Associations[0]
                    ...
                    const OfferPriceSegment =
                      OfferPrice.RequestedDate.Associations[AssociationIndex]
                        ?.ApplicableFlight.FlightSegmentReference[SegmentIndex];

                    if (
                      OfferPriceSegment &&
                      OfferPriceSegment.BagDetailAssociation
                    ) {
                        ...
                        if (filteredBagAllowance) {
                          ...
                          if (
                            ...
                            if (firstBaggage.WeightAllowance) {
                              const UOM =
                                firstBaggage.WeightAllowance.MaximumWeight[0]
                                  .UOM;
                              ...
                              baggage_unit = 'pieces';
                              baggage_count =
                                firstBaggage.PieceAllowance[0].TotalQuantity;
                            }
                          } else {
                            if (firstBaggage.WeightAllowance) {
                              const UOM =
        ...
        const FormattedPassengers = Offer.PricedOffer.OfferPrice.map(
          (OfferPrice) => {
            ...
            total_amount +=
              PriceDetail.TotalAmount.SimpleCurrencyPrice.value *
              paxCount *
              api_currency;
            ...
            const FormattedPassenger = {
              ...
              fare: {
                ...
                total_fare:
                  Number(PriceDetail.TotalAmount.SimpleCurrencyPrice.value) *
                  api_currency,
              },
            };
        ...
        // ticket last date
        const [ticket_last_date, ticket_last_time] =
          new FlightUtils().utcToLocalDateTime(
        ...
        let refundable;
        {
          const PenaltyRefs = [
            ...new Set(
              Offer.PricedOffer.OfferPrice.map((OP) =>
                OP.FareDetail.FareComponent.map(
                  (FC) => FC.FareRules?.Penalty.refs
                )
              )
                .flat(10)
          ...
          if (PenaltyRefs?.length && Penalty) {
            const matched = Penalty.filter(
              (item) =>
                PenaltyRefs.includes(item.ObjectKey) &&
                item.RefundableInd !== undefined
            );
          ...
          for (const detail of ChangeDetails) {
            ...
            if (Amounts) {
              for (const Amount of Amounts) {
                ...
                const decimalKey =
                  Offer.OfferID.Owner + '-' + changeFeeCurrencyCode;
          ...
          for (const detail of ChangeDetails) {
            ...
            if (Amounts) {
              for (const Amount of Amounts) {
                ...
                const decimalKey =
                  Offer.OfferID.Owner + '-' + cancelFeeCurrencyCode;
  ...
  public async FlightRevalidateService({
    ...
    FlightPriceRQ = await getRedis(
      `VerteilFlightPriceRQ-${search_id}-${oldData.flight_id}`
    );

    if (FlightPriceRQ === null) {
      const flightPriceRQs = await getRedis(
        `VerteilFlightPriceRQs-${search_id}`
      );
      ...
      FlightPriceRQ =
        flightPriceRQs?.find(
          (RQs: { flight_id: string }) => RQs.flight_id === oldData.flight_id
        )?.flightPriceRQ || null;
    }
    ...
    // Post Revalidate Works
    {
      ...
      const FlightPriceRQ =
        await new VerteilFlightUtils().PrepareMetaDataFlightPricePlus(
          oldData,
          reqBody,
          FlightPriceRS
        );
      ...
      const OrderCreateRQ =
        await new VerteilFlightUtils().PrepareMetaDataForOrderCreate(
          FlightPriceRS
        );
      setRedis(
        `VerteilOrderCreateRQ-${search_id}-${newData.flight_id}`,
        OrderCreateRQ,
        900
      );

      setRedis(
        `FlightRevalidateRS-${search_id}-${newData.flight_id}`,
        newData,
        900
      );
    }
  ...
  private async FlightRevalidateResFormatter({
    ...
    {
      const Associations =
        PricedFlightOffer[0].OfferPrice[0].RequestedDate.Associations;
      const FormattedAvailability = Associations.map(
        // Flight LEG
        (Association, AssociationIndex) => {
          const originDestRef =
            Association.ApplicableFlight.OriginDestinationReferences?.[0];
          const OriginDestination =
            response.DataLists.OriginDestinationList.OriginDestination.find(
              (od) => od.OriginDestinationKey === originDestRef
            );
              ...
              const FormattedPassengers = PricedFlightOffer[0].OfferPrice.map(
                ...
                (OfferPrice) => {
                  ...
                  if (
                    ...
                    if (CheckedBagReferences) {
                      const filteredBagAllowance =
                        DataLists.CheckedBagAllowanceList?.CheckedBagAllowance?.filter(
                      ...
                      if (filteredBagAllowance) {
                        ...
                        if (
                          ...
                          if (firstBaggage.WeightAllowance) {
                            const UOM =
                              firstBaggage.WeightAllowance.MaximumWeight[0].UOM;
                            baggage_unit = UOM === 'Kilogram' ? 'KG' : UOM;
                            baggage_count =
                              firstBaggage.WeightAllowance.MaximumWeight[0]
                                .Value;
                  ...
                  const FormattedPassenger: IFlightDataAvailabilityPassenger = {
                    ...
                    cabin_code:
                      OfferPriceSegment.ClassOfService.MarketingName
                        ?.CabinDesignator,
                    cabin_type:
                      OfferPriceSegment.ClassOfService.MarketingName?.value,
                    booking_code: OfferPriceSegment.ClassOfService.Code?.value,
                    available_seat:
                      Association.AssociatedService?.SeatAssignment?.length,
                    meal_code: undefined,
                    meal_type: undefined,
                    available_break: undefined,
                    available_fare_break: undefined,
        ...
        (OfferPrice) => {
          const travelerRef =
            OfferPrice.RequestedDate.Associations[0].AssociatedTraveler
              .TravelerReferences[0];
          const TravelerInfo =
            response.DataLists.AnonymousTravelerList?.AnonymousTraveler.find(
              (tr) => tr.ObjectKey === travelerRef
            );
          ...
          totalDiscount += discount * paxCount * api_currency;
          total_amount +=
            PriceDetail.TotalAmount.SimpleCurrencyPrice.value * paxCount;
  ...
  }: // primeFlow,
  {
    ...
    // Reorder passengers array to match incompletePax
    {
      ...
      const primaryAdultIndex = adults.findIndex(
        (p) => p.contact_number && p.contact_email
      );
    ...
    const completedPax = incompletePax.map((iPax, iPaxInd) => {
      ...
      if (paxType === 'INF') {
        ...
        FormattedPassenger.PassengerAssociation =
          incompletePax[infAssociationAdtIndex].ObjectKey;
        ...
        PassengerDocument: [
          {
            ...
            Type: 'PT',
            CountryOfIssuance:
              countries.find((cn) => cn.id == passenger.nationality)?.iso ||
              'BD',
            CountryOfResidence:
              countries.find((cn) => cn.id == passenger.nationality)?.iso ||
              'BD',
  ...
  public async FlightBookService({
    ...
    const preOrderCreateRQ = await getRedis(
      `VerteilOrderCreateRQ-${search_id}-${flight_id}`
    );
    ...
    const countries = await this.Model.CommonModel().getCountry({
    ...
    if (paymentTimeLimit === '' && Order?.OrderItems?.OrderItem[0].TimeLimits) {
      let timeLimit =
        Order.OrderItems.OrderItem[0].TimeLimits.PaymentTimeLimit?.Timestamp;
      if (!timeLimit)
        timeLimit =
          Order.OrderItems.OrderItem[0].TimeLimits.PriceGuaranteeTimeLimits
            ?.Timestamp;
      if (timeLimit)
  ...
  public async OrderRetrieveService({
    ...
    try {
      const OrderRetrieveRQ = this.orderRetrieveRequestFormatter(
        pnr,
        airlineCode
      );
      ...
      if (!OrderRetrieveRS)
        throw new CustomError('No information has been found', 404);
      ...
      if (
        paymentTimeLimit === '' &&
        Order?.OrderItems?.OrderItem[0].TimeLimits
      ) {
        let timeLimit =
          Order.OrderItems.OrderItem[0].TimeLimits.PaymentTimeLimit?.Timestamp;
        if (!timeLimit)
      ...
      const TicketDocInfo =
        OrderRetrieveRS.Response.TicketDocInfos?.TicketDocInfo;
      ...
      if (TicketDocInfo) {
        flightTickets = passengers.map((Pax) => {
          ...
          const paxRef = OrderRetrieveRS.Response?.Passengers.Passenger.find(
            (VerteilPax: {
              PassengerIDInfo: { PassengerDocument: { ID: string }[] };
            }) =>
              VerteilPax.PassengerIDInfo.PassengerDocument[0].ID.toUpperCase() ===
              passportID
          )?.ObjectKey;

          const TicketInfo = TicketDocInfo.find(
            (TDI: { PassengerReference: any[] }) =>
              TDI.PassengerReference[0] === paxRef
          );
      ...
      }>;
      {
        ...
        const PTCs = Array.from(
          new Map(Passenger.map((p: { PTC: any }) => [p.PTC, p])).values()
        );
        ...
        BaggagePerPTC = PTCs.map((PTC: any) => {
          const serviceRefs = Order.OrderItems?.OrderItem.find(
            (OI: {
              ...
              OI.Associations?.Passengers.PassengerReferences.includes(
                PTC.ObjectKey
              )
          )?.BaggageItem?.refs;
          ...
          const Service = DataLists.ServiceList?.Service.filter(
            (S: { ObjectKey: any }) => serviceRefs.includes(S.ObjectKey)
          ...
          const CarryOnAllowanceList =
            DataLists.CarryOnAllowanceList?.CarryOnAllowance.filter(
              (COA: { ListKey: any }) => ServiceRefs.includes(COA.ListKey)
            );

          const CheckedBagAllowanceList =
            DataLists.CheckedBagAllowanceList?.CheckedBagAllowance.filter(
              (CBA: { ListKey: any }) => ServiceRefs.includes(CBA.ListKey)
            );
  ...
  public async TicketIssueService({
    ...
    try {
      ...
      const { OrderReshopRS, OrderReshopRQ } = await this.OrderReshop({
        pnr,
        airlineCode,
      });
      {
        ...
        if (!OrderChangeRS || OrderChangeRS.Errors || !OrderChangeRS.Response) {
          await this.Model.ErrorLogsModel().insertErrorLogs({
            ...
            metadata: {
              api: VERTEIL_API,
              endpoint:
                VerteilAPIEndpoints.ACCEPT_REPRICE_OFFER_HEADER_ENDPOINT,
      ...
      const TicketDocInfo =
        OrderChangeRS.Response.TicketDocInfos?.TicketDocInfo;
  ...
  private async AcceptRepricedOffer({
    ...
    pnr: string;
    airlineCode: string;
    OrderReshopRS: IVerteilOrderReshopRS;
  }) {
  ...
  // Order cancel
  public async OrderCancelService({
    pnr,
    airlineCode,
  }: {
    pnr: string;
    ...
    try {
      ...
      if (OrderCancelRS.Errors)
        throw new Error(
          OrderCancelRS.Errors.Error.map(
            (Er: { value: string }) => Er.value + ' '
          ).join(',')
        );
