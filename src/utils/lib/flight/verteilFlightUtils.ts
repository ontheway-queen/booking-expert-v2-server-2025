...
import { PTC_TYPES_CHILD } from '../../miscellaneous/flightConstant';
...
export class VerteilFlightUtils {
  ...
  public async PrepareMetaDataForFlightPrice(
    ...
    AirShoppingRQ: Readonly<IVerteilFlightSearchRQ>
  ) {
    return await Promise.all(
      FormattedResults.map((Result, ResultIndex) => {
        ...
        const VerteilFlightPriceRequestBody: IVerteilFlightPriceRQ = {
          ShoppingResponseID,
          Query,
          DataLists: {
  ...
  //prepare data for storing into the redis
  public async PrepareMetaDataForOrderCreate(Data: IVerteilFlightPriceRS) {
    const PricedFlightOffer = Data.PricedFlightOffers.PricedFlightOffer;
