...
export class PublicPaymentService extends AbstractServices {
    ...
    public async transactionUsingSSL(req: Request) {
        return await this.db.transaction(async (trx) => {
            let { s_page = '', f_page = '', page = '', status = '' } = req.query as Record<string, string>;
            const body = req.body;
            ...
            switch (status) {
                case 'success':
                    ...
                    if (formatted_order_id[0] === SOURCE_AGENT) {
                        ...
                        const session = await axios.post(`${config.SSL_URL}/validator/api/validationserverAPI.php?val_id=${body?.val_id}&store_id=${config.SSL_STORE_ID}&store_passwd=${config.SSL_STORE_PASSWORD}&format=json`);
                        ...
                        await paymentModel.insertAgencyLedger(
                            {
                                ...
                                voucher_no: await Lib.generateNo({ trx, type: GENERATE_AUTO_UNIQUE_ID.agent_deposit_request })
                            }
                        );
