...
import { FLIGHT_BOOKING_IN_PROCESS, FLIGHT_FARE_RESPONSE, FLIGHT_REVALIDATE_REDIS_KEY, SABRE_API, WFTT_API } from "../../../utils/miscellaneous/flightConstent";
...
import { INVOICE_REF_TYPES, SOURCE_AGENT_B2C } from "../../../utils/miscellaneous/constants";
export class AgentB2CFlightService extends AbstractServices {
    ...
    public async flightSearch(req: Request) {
        return this.db.transaction(async (trx) => {
            ...
            //get b2c markup
            const markup_amount = await Lib.getAgentB2CTotalMarkup({ trx, type: 'Flight', agency_id });
            if (!markup_amount) {
                return {
                    success: false,
                    code: this.StatusCode.HTTP_BAD_REQUEST,
    ...
    public async getAllBookingList(req: Request) {
        return await this.db.transaction(async (trx) => {
            ...
            const data = await flightBookingModel.getFlightBookingList({ ...query, source_id: agency_id, created_by: user_id, booked_by: SOURCE_AGENT_B2C }, true);
