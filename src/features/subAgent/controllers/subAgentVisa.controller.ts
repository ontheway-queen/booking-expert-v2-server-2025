...
import { subAgentVisaService } from '../services/subAgentVisa.service';
import { SubAgentVisaValidator } from '../utils/validator/subAgentVisa.validator';

export class SubAgentCVisaController extends AbstractController {
  private service = new subAgentVisaService();
  private validator = new SubAgentVisaValidator();
  ...
  public getAllVisaCreatedCountry = this.asyncWrapper.wrap(
    null,
    async (req: Request, res: Response) => {
      const { code, ...rest } = await this.service.getAllVisaCreatedCountry(
        req
      );
      res.status(code).json(rest);
    }
  ...
  public getSingleVisaApplication = this.asyncWrapper.wrap(
    ...
    async (req: Request, res: Response) => {
      const { code, ...rest } = await this.service.getSingleVisaApplication(
        req
      );
      res.status(code).json(rest);
    }
