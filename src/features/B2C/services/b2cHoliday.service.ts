...
import {
  IGetSingleHolidayPackageByB2CParams,
  IHolidayPackageBookByB2CPayload,
} from '../utils/types/b2cHoliday.types';
...
export class B2CHolidayService extends AbstractServices {
  ...
  public async getSingleHolidayPackage(req: Request) {
    return await this.db.transaction(async (trx) => {
      const { slug } =
        req.params as unknown as IGetSingleHolidayPackageByB2CParams;
      const holidayPackageModel = this.Model.HolidayPackageModel(trx);
      console.log({ slug });
  ...
  public async bookHolidayPackage(req: Request) {
    return await this.db.transaction(async (trx) => {
      ...
      const price_details = get_holiday_data.pricing.find(
        (item) => item.price_for === SOURCE_B2C
      );
      if (!price_details) {
        return {
          success: false,
      ...
      const booking_ref = await Lib.generateNo({
        trx,
        type: GENERATE_AUTO_UNIQUE_ID.user_holiday,
      });
