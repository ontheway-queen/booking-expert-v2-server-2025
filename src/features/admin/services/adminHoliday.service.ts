...
import {
  ICreateHolidayReqBody,
  IUpdateHolidayPackageReqBody,
} from '../utils/types/adminHoliday.types';
    ...
    return await this.db.transaction(async (trx) => {
      ...
      if (pricing) {
        if (pricing.delete?.length) {
          await holidayPackagePricingModel.deleteHolidayPackagePricing(
            pricing.delete
          );
        }
          ...
          await Promise.all(
            pricing.add.map(async (item) => {
              const checkDuplicateEntry =
                await holidayPackagePricingModel.getHolidayPackagePricingList({
                  holiday_package_id: Number(id),
