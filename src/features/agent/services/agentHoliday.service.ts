...
import { GENERATE_AUTO_UNIQUE_ID, SOURCE_AGENT } from "../../../utils/miscellaneous/constants";
import { IGetSingleHolidayPackageByAgentParams, IHolidayPackageBookByAgentPayload } from "../utils/types/agentHoliday.types";
...
export class AgentHolidayService extends AbstractServices {
    ...
    public async bookHolidayPackage(req: Request) {
        return await this.db.transaction(async (trx) => {
            ...
            const get_holiday_data = await holidayPackageModel.getSingleHolidayPackage({ id: body.holiday_package_id, created_by: HOLIDAY_CREATED_BY_ADMIN, holiday_for: SOURCE_AGENT });
            ...
            //calculate the price
            const price_details = get_holiday_data.pricing.find((item) => item.price_for === SOURCE_AGENT);
            if (!price_details) {
                return {
                    success: false,
            ...
            const booking_ref = await Lib.generateNo({ trx, type: GENERATE_AUTO_UNIQUE_ID.agent_holiday });
