...
import { IAgencyB2CUpdateHolidayPackageReqBody, ICreateAgencyB2CHolidayReqBody } from "../../utils/types/agentB2CTypes/agentB2CSubHoliday.types";
import { HOLIDAY_CREATED_BY_AGENT, HOLIDAY_FOR_AGENT_B2C } from "../../../../utils/miscellaneous/holidayConstants";
...
export class AgentB2CSubHolidayService extends AbstractServices {
    ...
    public async createHoliday(req: Request) {
        return await this.db.transaction(async (trx) => {
            ...
            //check slug
            const slugCheck = await holidayPackageModel.getHolidayPackageList({ slug: rest.slug, created_by: HOLIDAY_CREATED_BY_AGENT, agency_id });
            ...
            //insert pricing
            await holidayPackagePricingModel.insertHolidayPackagePricing({ ...pricing, holiday_package_id: holidayPackage[0].id, price_for: HOLIDAY_FOR_AGENT_B2C });
        ...
        return await this.db.transaction(async (trx) => {
            ...
            if (pricing) {
                if (pricing.update) {
                    ...
                    await holidayPackagePricingModel.updateHolidayPackagePricing(rest, id)
                }
            }
